<template>
<div id="fog" v-bind:style="{ 'background-color': 'rgba(122, 114, 107,' + {{ weather }} + ');' }">
  <p>{{ title }}</p>
  <p>{{ weather }}°</p>
    ...
    computed : {
      dynamicStyle() {
          return {
              ...
              color : `rgb(${this.redComp}, 0, 0);`,
          };
      },
    },
    mounted () {
      axios
        .get('https://api.wunderground.com/api/89c63e6a34bf4afb/geolookup/conditions/q/zmw:95497.1.99999.json')
        .then(response => (this.weather = response.data.current_observation.temp_f))

      var temp = this;
      setInterval(function(){
          axios.get('https://api.wunderground.com/api/89c63e6a34bf4afb/geolookup/conditions/q/zmw:95497.1.99999.json')
            .then(response => (temp.weather = response.data.current_observation.temp_f))
